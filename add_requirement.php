...
// Admin can post unlimited req (all types)
// Dealer and member can post 30 reqs (including 6 super hot & 9 hot adds)
// Office member can post unlimited req (including 6 super hot & 9 hot adds)
// Common user can post 1 req
...
if($role == 'dealer' || $role == 'member' || $role == 'office')
{
        ...
        // if user id Dealer or member
        if($_SESSION['role'] != 'office' && $row['No_of_Req'] >= 30)
        {
                ?>
                <script>
                        window.location = "pop_up.php?id=req_limit_exceed";
                </script>
                <?php
        }
        else
...
//         {
                ...
                <?php
//         }
//         else if(($role == 'dealer' || $role == 'member') && $row['No_of_Req'] >= 7)
//         {
                ?>
                <!-- <script>
                        window.location = "pop_up.php?id=req_limit_exceed";
                ...
                <?php
//         }
//         else if($role == 'office' && $row['No_of_Req'] >= 15)
//         {
                ?>
                <!-- <script>
                        window.location = "pop_up.php?id=req_limit_exceed";
...
$everyThingOK = true;
$count = 0;
$agent_user_name = $req_id = $address_id = $marla = $road = $kanal = $acre = $length = $width = $min_range = $max_range = $type = $req_type = "";
$size_err = $city_err = $province_err  = $area_err = $comment_err = $phone_err  = "";
     ...
     if($connection)
      {
             ...
             $row = mysqli_fetch_assoc($result);
                ...
                if($everyThingOK)
                {
                        ...
                        {
                                if($_POST['req_type'] == 'super_hot')
                                {
                                        $req_type = 'super_hot';
                                }
                                else if($_POST['req_type'] == 'hot')
                                {
                                        $req_type = 'hot';
                                }
                                else if($_POST['req_type'] == 'normal')
                                {
                                        $req_type = 'normal';
                                }
                        }
                        ...
                        $min_range = mysqli_real_escape_string($connection, $min_range);
                        $max_range = mysqli_real_escape_string($connection, $max_range);
                        ...
                        if ($count == 0 && $_SESSION['role']!='user' && $_SESSION['role'] != 'dealer')
                        {
                                ?>
                                <script>
                                        window.location = "pop_up.php?id=clientNot_exist_req&cus_id=<?php echo $client_phone?>&type=<?php echo $_GET['type'];?>";
                                </script>
                                <?php
...
//                                $_POST['name']="";
                        ...
                        // if client exist then save data to db
                        else
                        {

                                $date = date("Y-m-d H:i:s", time());
                                $query="INSERT INTO requirement(Agent_ID, Marla, Kanal, Acre, Width, Length, Facing_Road, Min_Range, Max_Range, Area, City, Province, Type, Req_Type, Comment, Status, Show_Requirement, Date)
                                VALUES('$agent_user_name','$marla','$kanal','$acre','$width','$length','$road','$min_range','$max_range','$area','$city','$province','$type','$req_type','$comment','1', '1','$date')";
                                $result=mysqli_query($connection,$query)or die(mysqli_error($connection));



                                // get current plot ID (to send to pop up)
                                $result2 = mysqli_query($connection, "SELECT Requirement_ID FROM requirement");
                                ...
                                // // plot_address
                                // $query3="INSERT INTO plot_address(Plot_ID, Address_ID) VALUES('$req_id','$address_id')";
                                // $result5=mysqli_query($connection,$query3)or die(mysqli_error($connection));

                                if($_SESSION['role']!='user' && $_SESSION['role'] != 'dealer')
                                {
                                        // client_plot
                                        $query2="INSERT INTO client_requirement(Client_Phone, Requirement_ID) VALUES('$client_phone','$req_id')";
                                        $result3=mysqli_query($connection,$query2)or die(mysqli_error($connection));
                                }
                                // else
                                // {
                                //         // user_plot
                                //         $query2="INSERT INTO user_plot(Agent_ID, Plot_ID) VALUES('$agent_user_name','$req_id')";
                                //         $result3=mysqli_query($connection,$query2)or die(mysqli_error($connection));
                                // }
                                if($result)             // if operation successful
                                {
                                                $query = "SELECT No_of_Req FROM user WHERE Agent_ID = '$agent_user_name'";
                                                $result = mysqli_query($connection, $query)or die(mysqli_error($connection));
                                                $row = mysqli_fetch_assoc($result);
                                                $num = $row['No_of_Req'];
                                                $num++;
                                                                                                
                                                $query = "UPDATE user SET No_of_Req = '$num' WHERE Agent_ID = '$agent_user_name'";
                                                $result=mysqli_query($connection, $query)or die(mysqli_error($connection));
                                        ?>
                                                <script>
                                                        window.location = "pop_up.php?id=req_id&value=<?php echo $req_id?>&type=<?php echo $_GET['type'];?>";
                                                </script>
                                        <?php
                                }
                                else
                ...
                }       // if every thing ok check ends
           
       }        // end of if($connection)
}       // end of if(isset($_POST["add_req"]))
?>

<div class="container">
    
    <form class="two_row_form" action="add_requirement.php?type=<?php echo $_GET['type']?>" method="post">
                      ...
                      else if($_GET['type']=='industrial' || $_GET['type']=='agricultural')
                      {
                          ...
                          <!-- Left Side of 1st Row -->
                              <div class="col-lg-2">
                                      ...
                                      <label for=" ">Land Size<span class="error"></span></label>
                              </div>
                              <div class="col-lg-4">
        ...
        <!-- 2nd row -->
        <?php
        if($_GET['type']=='industrial' || $_GET['type']=='agricultural')
        {
        ?>
             <div class="row">  
        ...
        <!-- 3rd row -->
            <div class="row">                    
                    ...
                    <div class="col-lg-6">
                        <label for=" ">Facing Road Size </label>
            ...
            <div class="row">
                <div class="col-lg-3">
                    ...
                    <div class="col-lg-3">
                        ...
                        <input style="background-color : white; border: 1px solid silver;"  type="text" class="form-control" id=" " name="min_range" placeholder="Rs:">
                    </div>
                    <div class="col-lg-3">
                            <label for=" ">Maximum Price</label>
                            <input style="background-color : white; border: 1px solid silver;"  type="text" class="form-control" id=" " name="max_range" placeholder="Rs:">
                    </div>
            </div>
            ...
            <button type="submit" class="btn btn-success mb-2" name="add_req">Add Requirement</button>
