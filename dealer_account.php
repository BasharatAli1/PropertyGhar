...
$everyThingOK = true;
$phone_p_id = $phone_o_id = $address_id = "";
$name_err = $f_name_err = $u_name_err = $pass_err = $phone_o_err = $city_err = $province_err = $address_err = $gender_err  = $estate_err = "";

if(isset($_POST["dealer_account"]))
{
...
//        $connection=mysqli_connect("localhost:3306", "proper10_db_login", "~s*}t&~}ZuC{1", "proper10_propertyghar");
        ...
        if($connection)
        {
                ...
                if (empty($_POST["name"])|| empty($_POST["fatherName"])||empty($_POST["userName"])||empty($_POST["password"])||empty($_POST["city"])||empty($_POST["province"])||empty($_POST["address"])||empty($_POST["estate"]))
                {
                        ?>
                        <div class="container">
                        <div class="row">
                ...
                if (strlen($_POST["phone_office"]) < 10)
                        $phone_o_err=" uncomplete number";
                if (empty($_POST["name"]))
                        $name_err=" Required";
                ...
                if($everyThingOK)
                {
                        ...
                        else
                        {
                                ...
                                */
                                ...
                                $phone_o = mysqli_real_escape_string($connection, $phone_o);
                                $city = mysqli_real_escape_string($connection, $city);
                                $province = mysqli_real_escape_string($connection, $province);
                                $address = mysqli_real_escape_string($connection, $address);
                                $estate = mysqli_real_escape_string($connection, $estate);
                                $area = mysqli_real_escape_string($connection, $area);
                        


                                // // personal contact
                                // $query="INSERT INTO contact(Phone, Type) VALUES('$phone_p','personal')";
                                // $result1=mysqli_query($connection,$query);

                                // $result2 = mysqli_query($connection, "SELECT * FROM contact");
                                // while($row = mysqli_fetch_assoc($result2))
                                // {
                                //         $phone_p_id = $row['Contact_ID'];
                                // }

                                // // user phone
                                // $query="INSERT INTO user_contact(Agent_ID, Contact_ID) VALUES('$u_name','$phone_p_id')";
                                // $result3=mysqli_query($connection,$query);

                                // // office contact
                                // if(!empty($phone_o))
                                // {
                                //         $query="INSERT INTO contact(Phone, Type) VALUES('$phone_o','office')";
                                //         $result4=mysqli_query($connection,$query);
                                        
                                //         $result5 = mysqli_query($connection, "SELECT * FROM contact");
                                //         while($row = mysqli_fetch_assoc($result5))
                                //         {
                                //                 $phone_o_id = $row['Contact_ID'];
                                //         }

                                //         // user phone
                                //         $query="INSERT INTO user_contact(Agent_ID, Contact_ID) VALUES('$u_name','$phone_o_id')";
                                //         $result6=mysqli_query($connection,$query);
                                // }
                                
                                // // address
                                // $query="INSERT INTO address(City, Province, Address) VALUES('$city','$province','$address')";
                                // $result7=mysqli_query($connection,$query);

                                // // user address
                                // $result8 = mysqli_query($connection, "SELECT * FROM address");
                                // while($row = mysqli_fetch_assoc($result8))
                                // {
                                //         $address_id = $row['Address_ID'];
                                // }
                                // $query="INSERT INTO user_address(Agent_ID, Address_ID) VALUES('$u_name','$address_id')";
                                ...
                                VALUES('$name','$father_name','$cnic','$u_name','$pass','$email','$estate','$gender','dealer','$phone_p','$phone_o','$city','$province','$address','1','0','0','0','0','0')";

                                $result=mysqli_query($connection,$query)or die(mysqli_error($connection));

                                /* Use \ or / or , or . characters to tokenize  */
                                $tok = strtok($area, "\\,/.");

                                while ($tok !== false) 
                                {
                                        $query = "INSERT INTO area(Area) VALUES ('$tok')";
                                        $result10 = mysqli_query($connection, $query)or die(mysqli_error($connection));
                                                
                                                $query2 = "SELECT * FROM area where Area = '$tok'";
                                                $result11 = mysqli_query($connection, $query2)or die(mysqli_error($connection));
                                                while($row = mysqli_fetch_assoc($result11))
                                                {
                                                        $area_id=$row['Area_ID'];
                                                }
                                                $query3 = "INSERT INTO user_area(Agent_ID, Area_ID) VALUES ('$u_name', '$area_id')";
                                                $result12 = mysqli_query($connection, $query3)or die(mysqli_error($connection));
...
<div class="container">
    ...
    <form class="two_row_form" action="dealer_account.php" method="post">
            ...
            <div class="row">
                    <div class="col-lg-6">
                            <label for=" ">Phone # (Personal)</label>
                            <input style="background-color : white; border: 1px solid silver;"  type="text" class="form-control" id=" " value="" name="phone_personal" placeholder="Phone (personal)">
                    </div>
                    <div class="col-lg-6">
                            <label for=" ">Phone # (Office)<span class="error">*<?php echo $phone_o_err;?></span></label>
                            <input style="background-color : white; border: 1px solid silver;"  type="text" class="form-control" id=" " name="phone_office" placeholder="Phone (office)">
                    </div>
            </div>
            ...
            <div class="row">
                    <div class="col-lg-6">
                            <label for=" ">Estate Name<span class="error">*<?php echo $estate_err;?></span></label>
                            <input style="background-color : white; border: 1px solid silver;"  type="text" class="form-control" id=" " name="estate" placeholder="Estate Names">
                    </div>
                    <div class="col-lg-6">
                        <label for=" ">Societies/Areas</label>
                        <input style="background-color : white; border: 1px solid silver;"  type="text" class="form-control" id=" " name="area" placeholder=' Comma "," seperated Area Names'>
                    </div>
            </div>
            <br>

            <button type="submit" class="btn btn-success" name="dealer_account">Create Account</button>
    </form>
