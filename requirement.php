        ...
        <ul class="navbar-nav">
              ...
              <?php
                if(!empty($_SESSION["role"]) && $_SESSION["role"] !='user')
                {
                  ?>
              <li class="nav-item">
                  <a style="font-size:18px;" class="nav-link active" href="requirement.php"> <img style = "width:20px;" src="images/navbar/requirement.png" alt="adds"> Requirement </a>
              </li>
              <?php
              }
                ?>
...
<br>
        ...
        $dealer_contact = $user_contact = $agent_id = $role = $min_range = $max_range = $req_id = $phone = $province = $city = $area = $status = $link = $area = $lw = $road = "";
          
        if($s_area != "" && $s_city == "")
        {
    //      $query = "SELECT * FROM requirement WHERE Status = '1' AND Show_Requirement = '1' AND Area = '$s_area' ORDER BY Date DESC";
          $query = "SELECT * FROM requirement WHERE Status = '1' AND Show_Requirement = '1' AND Area LIKE '%{$s_area}%' ORDER BY Date DESC";
          $result = mysqli_query($connection, $query)or die(mysqli_error($connection));
        }
        else if($s_city != "" && $s_area == "")
        {
          $query = "SELECT * FROM requirement WHERE Status = '1' AND Show_Requirement = '1' AND City LIKE '%{$s_city}%' ORDER BY Date DESC";
          $result = mysqli_query($connection, $query)or die(mysqli_error($connection));
        }
        else if($s_area != "" && $s_city != "")
        {
          $query = "SELECT * FROM requirement WHERE Status = '1' AND Show_Requirement = '1' AND Area LIKE '%{$s_area}%' AND City LIKE '%{$s_city}%' ORDER BY Date DESC";
          $result = mysqli_query($connection, $query)or die(mysqli_error($connection));
        }
        else
        {
          $query = "SELECT * FROM requirement WHERE Status = '1' AND Show_Requirement = '1' ORDER BY Date DESC";
          $result = mysqli_query($connection, $query)or die(mysqli_error($connection));
        }
    ...
    ?>
                  <div class = "container">
                  <h3 style="text-align:center; font-weight:bold; margin-top:1%;"> </h3>
              ...
              <center><?php echo $size;?>, <?php echo $type ?></center>
                      <div class = "row">  <!-- Price & Type -->
                          ...
                          <div class="col-lg-5">                
                            <span style="font-weight:bold;">Range:</span> <?php echo 'Rs: ' . $min_range . ' - ' . $max_range?>
                          </div>
                          <div class="col-lg-5">          
